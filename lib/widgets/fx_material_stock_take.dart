...
import '../screen/stock_take/stock_take_save_provider.dart';
import 'fx_gray_dark_text.dart';
import 'fx_green_dark_text.dart';
import 'fx_text_field.dart';
import 'dart:html' as html;

class FxMaterialStockTake extends HookConsumerWidget {
  final bool isFirst;
  final bool inEditMode;
  final int index;
  final GroupSummaryModelV2 model;
  final TextEditingController ctrlEdit;
  final void Function(
          GroupSummaryModelV2 model, String strQty, bool status, String message)
      onSave;

  const FxMaterialStockTake({
    super.key,
    required this.model,
    this.isFirst = false,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen(stockTakeSaveProvider, (prev, next) {
      if (next is StockTakeSaveStateLoading) {
        isLoading.value = true;
      } else if (next is StockTakeSaveStateDone) {
        isLoading.value = false;
      } else if (next is StockTakeSaveStateError) {
                ...
                Expanded(
                  ...
                  child: Column(children: [
                    Align(
                      ...
                      child: FxGreenDarkText(title: "Scan Qty :"),
                    ),
                    SizedBox(height: 10),
                    Align(
                      alignment: Alignment.centerLeft,
                ...
                if (isInIssueLooseQty.value)
                  InkWell(
                    onTap: () {
                      ...
                      var newQty = 0.0;
                      try {
                        newQty = double.parse(ctrlEdit.text);
                        var oldQty = double.parse(model.scanQty ?? "0");
                        if (newQty > oldQty) {
                          errorMessage.value = "New Qty > Old Qty";
                          Timer(Duration(seconds: 3), () {
                            errorMessage.value = "";
                            var nbf = NumberFormat("##0");
                            ctrlEdit.text = nbf.format(oldQty);
                          });
                          return;
                        }
                      } catch (_) {
                      ...
                      ref.read(stockTakeSaveProvider.notifier).save(
                            stockTakeID: model.stockTakeID ?? "0",
                            stockTakeItemID: model.stockTakeItemID ?? "0",
                            scanQty: ctrlEdit.text,
                          );
