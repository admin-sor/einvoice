...
import '../screen/tx_in/tx_in_update_qty_provider.dart';
import 'fx_gray_dark_text.dart';
import 'fx_green_dark_text.dart';
import 'fx_text_field.dart';
import 'dart:html' as html;

class FxMaterialTxInInfo extends HookConsumerWidget {
  final bool isFirst;
  final bool inEditMode;
  final int index;
  final TxInListModel model;
  final String txInID;
  final TextEditingController ctrlEdit;
  final void Function(String txInID, TxInListModel model, String strQty,
      bool status, String message) onSave;
  final String slipNo;

  const FxMaterialTxInInfo({
    super.key,
    required this.txInID,
    required this.model,
    this.isFirst = false,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isInIssueLooseQty = useState(true);
                ...
                Expanded(
                  ...
                  child: Column(children: [
                    Align(
                      ...
                      child: FxGreenDarkText(title: "Edit Qty :"),
                    ),
                    SizedBox(height: 10),
                    Align(
                      alignment: Alignment.centerLeft,
                ...
                if (isInIssueLooseQty.value)
                  InkWell(
                    onTap: () {
                      ...
                      ref.read(txInUpdateQtyStateProvider.notifier).update(
                            slipNo: slipNo,
                            txInID: txInID,
                            barcode: model.txInBarcode ?? "",
                            qty: ctrlEdit.text,
                          );
                      onSave(txInID, model, ctrlEdit.text, true, "");
                    },
                    child: Center(
                      child: Image.asset(
