...
class FxScanInfo extends StatelessWidget {
  final bool isFirst;
  final BarcodeScanResponseModel model;
  const FxScanInfo({
    super.key,
    required this.model,
    this.isFirst = false,
  ...
  Widget build(BuildContext context) {
    return ResponsiveBuilder(builder: (context, sizeinfo) {
      ...
      String sdate = model.doDate;
      try {
        final sdf = DateFormat("yyyy-MM-dd");
        sdate = DateFormat("dd MMM yyyy").format(sdf.parse(sdate));
      } catch (_) {}
      return Card(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              _LeftColumn(label: "Date", value: sdate),
              _LeftColumn(label: "DO", value: model.doNo),
              _LeftColumn(label: "Vendor", value: model.vendorName),
